...
[Setup]
AppName=Vision Insight Web Application 
AppVersion=3.0.2
...
UninstallDisplayIcon={app}\Group_1.ico
...
[Icons]
; Start Menu shortcut
Name: "{group}\Vision Insight"; Filename: "http://localhost:8090"; IconFilename: "{app}\Group_1.ico"

; Desktop shortcut (optional)
Name: "{commondesktop}\Vision Insight"; Filename: "http://localhost:8090"; IconFilename: "{app}\Group_1.ico"

[Files]

Source: "Group_1.ico"; DestDir: "{app}"; Flags: ignoreversion
Source: "{src}\{#DependenciesDir}\dotnet-hosting-9.0.0-win.exe"; DestDir: "{tmp}"; Flags: external
Source: "{src}\{#DependenciesDir}\dotnet-sdk-8.0.412-win-x64.exe"; DestDir: "{tmp}"; Flags: external
Source: "{src}\{#DependenciesDir}\mongodb-windows-x86_64-7.0.12-signed.msi"; DestDir: "{tmp}"; Flags: external
Source: "{src}\{#DependenciesDir}\mongosh-2.3.0-x64.msi"; DestDir: "{tmp}"; Flags: external
Source: "{src}\{#DependenciesDir}\mongodb-database-tools-windows-x86_64-100.11.0.msi"; DestDir: "{tmp}"; Flags: external
...
begin
  ...
  if DirExists(SetupDir + '{#DependenciesDir}') then
  ...
  if FileExists(SetupDir + '{#DependenciesDir}\dotnet-hosting-9.0.0-win.exe') then
    DebugMsg := DebugMsg + '✓ dotnet-hosting-9.0.0-win.exe: FOUND' + #13#10
  else begin
    DebugMsg := DebugMsg + '✗ dotnet-hosting-9.0.0-win.exe: NOT FOUND' + #13#10;
    DebugMsg := DebugMsg + '  Expected at: ' + SetupDir + '{#DependenciesDir}\dotnet-hosting-9.0.0-win.exe' + #13#10;
  end;
  
  if FileExists(SetupDir + '{#DependenciesDir}\dotnet-sdk-8.0.412-win-x64.exe') then
    ...
    DebugMsg := DebugMsg + '  Expected at: ' + SetupDir + '{#DependenciesDir}\dotnet-sdk-8.0.412-win-x64.exe' + #13#10;
  end;
  
  if FileExists(SetupDir + '{#DependenciesDir}\mongodb-windows-x86_64-7.0.12-signed.msi') then
    ...
    DebugMsg := DebugMsg + '  Expected at: ' + SetupDir + '{#DependenciesDir}\mongodb-windows-x86_64-7.0.12-signed.msi' + #13#10;
  end;
  
  if FileExists(SetupDir + '{#DependenciesDir}\mongosh-2.3.0-x64.msi') then
    DebugMsg := DebugMsg + '✓ mongosh-2.3.0-x64.msi: FOUND' + #13#10
  else begin
    DebugMsg := DebugMsg + '✗ mongosh-2.3.0-x64.msi: NOT FOUND' + #13#10;
    DebugMsg := DebugMsg + '  Expected at: ' + SetupDir + '{#DependenciesDir}\mongosh-2.3.0-x64.msi' + #13#10;
  end;
  
   if FileExists(SetupDir + '{#DependenciesDir}\mongodb-database-tools-windows-x86_64-100.11.0.msi') then
    DebugMsg := DebugMsg + '✓ mongodb-database-tools-windows-x86_64-100.11.0.msi: FOUND' + #13#10
  else begin
    DebugMsg := DebugMsg + '✗ mongodb-database-tools-windows-x86_64-100.11.0.msi: NOT FOUND' + #13#10;
    DebugMsg := DebugMsg + '  Expected at: ' + SetupDir +'{#DependenciesDir}\mongodb-database-tools-windows-x86_64-100.11.0.msi' + #13#10;
  end;
  ...
  if not FileExists(SetupDir + '{#DependenciesDir}\dotnet-hosting-9.0.0-win.exe') then
  begin
    MsgBox('Required file missing: {#DependenciesDir}\dotnet-hosting-9.0.0-win.exe' + #13#10 + 
           'Please ensure all dependency files are in the Dependencies folder.', mbError, MB_OK);
    Result := False;
  end;
  
  if not FileExists(SetupDir + '{#DependenciesDir}\dotnet-sdk-8.0.412-win-x64.exe') then
  begin
    MsgBox('Required file missing: {#DependenciesDir}\dotnet-sdk-8.0.412-win-x64.exe' + #13#10 + 
           'Please ensure all dependency files are in the Dependencies folder.', mbError, MB_OK);
    Result := False;
  end;
  
  if not FileExists(SetupDir + '{#DependenciesDir}\mongodb-windows-x86_64-7.0.12-signed.msi') then
  begin
    MsgBox('Required file missing: {#DependenciesDir}\mongodb-windows-x86_64-7.0.12-signed.msi' + #13#10 + 
           'Please ensure all dependency files are in the Dependencies folder.', mbError, MB_OK);
    Result := False;
  end;
  
  if not FileExists(SetupDir + '{#DependenciesDir}\mongosh-2.3.0-x64.msi') then
  begin
    MsgBox('Required file missing: {#DependenciesDir}\mongosh-2.3.0-x64.msi' + #13#10 + 
           'Please ensure all dependency files are in the Dependencies folder.', mbError, MB_OK);
    Result := False;
  end;
  
  if not FileExists(SetupDir + '{#DependenciesDir}\mongodb-database-tools-windows-x86_64-100.11.0.msi') then
  begin
    MsgBox('Required file missing: {#DependenciesDir}\mongodb-database-tools-windows-x86_64-100.11.0.msi' + #13#10 + 
           'Please ensure all dependency files are in the Dependencies folder.', mbError, MB_OK);
    Result := False;
  end;
end;
...
begin
  Result := Assigned(CreateShortcutCheckBox) and CreateShortcutCheckBox.Checked;
end;
...
begin
  ...
  ShortcutPath := DesktopPath + '\Vision Insight.lnk';
  
  // Create batch file to launch localhost
  BatchFile := ExpandConstant('{app}\launch.bat');
  BatchContent := '@echo off' + #13#10 + 'start http://localhost:8090';
  ...
  Exec('powershell.exe', 
       ...
       '$Shortcut.IconLocation = ''' + ExpandConstant('{app}\Group_1.ico') + '''; ' +
...
begin
  ...
  begin
    if CreateShortcutCheckBox.Checked then
    begin
  ...
  begin
    if not (IISSetupCheckBox.Checked or NetHostingCheckBox.Checked or MongoDBCheckBox.Checked or 
            MongoToolsCheckBox.Checked) then
    begin
...
begin
  ...
  CheckCmd := 'try { ' +
              'if (Test-Path "C:\Program Files\MongoDB\Tools\") { "INSTALLED" | Out-File "' + TempFile + '" -Encoding UTF8; exit 0; } ' +
...
begin
  Result := False;
  SetupDir := ExtractFilePath(ExpandConstant('{srcexe}'));
  SourcePath := SetupDir + '{#DependenciesDir}\' + LocalFileName;
...
begin
  ...
  MongoBinPath := 'C:\Program Files\MongoDB\Server\7.0\bin';
  ...
  begin
   ...
   if not IsPathInEnvironment(MongoBinPath) then
    begin
    //MsgBox('Adding MongoDB Server to PATH...Updating environment variables... ' + MongoBinPath, mbInformation, MB_OK);
      ...
      if AddToSystemPath(MongoBinPath) then
        ...
        //MsgBox('Warning: IsPathInEnvironment method called: ' + MongoBinPath, mbInformation, MB_OK);
    end;
...
begin
  ...
  MongoshBinPath := 'C:\Program Files\MongoDB\Tools\100\bin';
  ...
  begin
    ...
    // Still check and add to PATH if needed
    
    if not IsPathInEnvironment(MongoshBinPath) then
    begin
    MsgBox('Adding MongoDB Tools to PATH...Updating environment variables... ' + MongoshBinPath, mbInformation, MB_OK);
      ...
      if AddToSystemPath(MongoshBinPath) then
        ...
        MsgBox('Warning: IsPathInEnvironment method called: ' + MongoshBinPath, mbInformation, MB_OK);
    end;
  ...
  InstallerPath := ExpandConstant('{tmp}\mongodb-database-tools-windows-x86_64-100.11.0.msi');
  MongoshPath := 'C:\Program Files\MongoDB\Tools\';
  ...
  begin
    ProgressPage.SetText('MongoDB Tools folder already exists.', 'Skipping installation to avoid error 1603...');
    ProgressPage.SetProgress(70, 100);
    Exit;
  end;
  
  // Copy local file
  if not CopyLocalFile('mongodb-database-tools-windows-x86_64-100.11.0.msi', InstallerPath) then
  begin
  ...
  // Add MongoDB Tools to system PATH
  ...
  begin
    //MsgBox('MongoDB Tools installed successfully, but could not add to system PATH automatically.' + #13#10 +
      //     'Please add the following path to your system PATH manually:' + #13#10 +
        //   MongoshBinPath, mbInformation, MB_OK);
  end;
...
begin
  ...
  // Path to your local mongosh MSI file (you can change this to {src}, {app}, etc.)
  MSIPath := ExpandConstant('{src}\mongosh-2.3.0-x64-2908.msi');


  if FileExists(MSIPath) then
  begin
    // Install silently
    if Exec('msiexec.exe', '/i "' + MSIPath + '" /qn INSTALLLOCATION="C:\Program Files\mongosh"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode) then
    begin
      Sleep(2000); // wait for file write

      if not FileExists(MongoShellPath) then
      begin
        ProgressPage.SetText('Failed to install mongosh.', 'Please install it manually from ' + MSIPath);
        Exit;
      end;
    end
    else
    begin
      ProgressPage.SetText('Mongosh installation failed.', 'Please install manually.');
      Exit;
    end;
  end
  else
  begin
    ProgressPage.SetText('Mongosh MSI not found at ' + MSIPath, 'Please ensure the file exists.');
    Exit;
  end;
end;
