...
function Install-DotNetSDK {
    ...
    $dotnetExe = Join-Path -Path $env:ProgramFiles -ChildPath "dotnet\dotnet.exe"
    Write-Host "dotnetExe =  $dotnetExe."

    if (-not (Test-Path $dotnetExe)) {
        Write-Error "dotnet.exe not found at $dotnetExe"
        return
    }
    ...
    $installedVersions = & $dotnetExe --list-sdks |
