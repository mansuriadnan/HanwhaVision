...
Source: "{src}\Dependencies\mongodb-database-tools-windows-x86_64-100.11.0.msi"; DestDir: "{tmp}"; Flags: external

;[Tasks]
;Name: "desktopicon"; Description: "{cm:CreateDesktopIcon}"; GroupDescription: "{cm:AdditionalIcons}"; Flags: unchecked

;[Icons]
; Desktop shortcut for Vision Insight Test
;Name: "{userdesktop}\Vision Insight Test"; Filename: "http://localhost:8080"; IconFilename: "{app}\Group_1.ico"; Tasks: desktopicon

; Alternative approach if you want to open localhost in default browser
; Name: "{userdesktop}\Vision Insight Test"; Filename: "{sys}\cmd.exe"; Parameters: "/c start http://localhost:8080"; IconFilename: "{app}\Group_1.ico"; Tasks: desktopicon; Comment: "Launch Vision Insight Test"

; If you want to create a batch file approach
; Name: "{userdesktop}\Vision Insight Test"; Filename: "{app}\launch.bat"; IconFilename: "{app}\Group_1.ico"; Tasks: desktopicon

;[Icons]
;Name: "{userdesktop}\Vision Insight"; Filename: "http://localhost:8090"; IconFilename: "{app}\Group_1.ico"; Check: ShouldCreateShortcut
...
begin
  ...
  if FileExists(SetupDir + 'Dependencies\mongosh-2.3.0-x64.msi') then
   ...
   if FileExists(SetupDir + 'Dependencies\mongodb-database-tools-windows-x86_64-100.11.0.msi') then
    ...
    DebugMsg := DebugMsg + '  Expected at: ' + SetupDir +'Dependencies\mongodb-database-tools-windows-x86_64-100.11.0.msi' + #13#10;
  end;
  ...
  if not FileExists(SetupDir + 'Dependencies\mongodb-database-tools-windows-x86_64-100.11.0.msi') then
  begin
    MsgBox('Required file missing: Dependencies\mongodb-database-tools-windows-x86_64-100.11.0.msi' + #13#10 + 
           'Please ensure all dependency files are in the Dependencies folder.', mbError, MB_OK);
    Result := False;
  end;
end;
...
begin
  ...
  // Create desktop shortcut checkbox for restart on finish page
...
begin
  ...
  CopyScript :=
    ...
    'try {' + #13#10 +
    '  Import-Module WebAdministration' + #13#10 +
    '  Write-Log "Stopping IIS App Pool: MyAppPool"' + #13#10 +
    '  Stop-WebAppPool "MyAppPool"' + #13#10 +
    '  Start-Sleep -Seconds 5' + #13#10 +
    '  Write-Log "Copying files from $SourceDir to $DestDir using robocopy"' + #13#10 +
    '  $params = @(' + #13#10 +
    ...
    '    exit $code' + #13#10 +
    '  }' + #13#10 +
    '  Write-Log "Starting IIS App Pool: MyAppPool"' + #13#10 +
    '  Start-WebAppPool "MyAppPool"' + #13#10 +
    '  Write-Log "IIS App Pool restarted successfully."' + #13#10 +
    '  exit 0' + #13#10 +
    '} catch {' + #13#10 +
    '  Write-Log "Exception occurred: $($_.Exception.Message)"' + #13#10 +
    '  exit 1' + #13#10 +
    '}';
