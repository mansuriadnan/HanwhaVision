...
function Install-AspNetCoreHostingBundle {
    param (
        [string]$DownloadUrl,
        [string]$InstallerName = "dotnet-sdk-9.0.102-win-x64.exe"
    )

    # Define the path where the installer will be saved
    $outputPath = "$env:TEMP\$InstallerName"
    $requiredVersion = "9.0.102"

    # Check if the required .NET SDK is already installed
    $installedSdks = & dotnet --list-sdks 2>$null | ForEach-Object { ($_ -split "\s+")[0] }

    if ($installedSdks -contains $requiredVersion) {
        Write-Host ".NET SDK $requiredVersion is already installed. Skipping installation."
        return
    }

    try {
        # Download the installer
        Write-Host "Downloading .NET SDK from $DownloadUrl..."
        Invoke-WebRequest -Uri $DownloadUrl -OutFile $outputPath

        # Install the SDK silently
        Write-Host "Installing .NET SDK $requiredVersion..."
        Start-Process -FilePath $outputPath -ArgumentList "/install", "/quiet", "/norestart" -Wait

        Write-Host ".NET SDK $requiredVersion installation completed successfully."
    }
    catch {
...
$PhysicalPath = "$PSScriptRoot\Publish"     # Path to the publish folder
...
#NetCore-Hosting URL
$NetHostingInstallerUrl = "https://download.visualstudio.microsoft.com/download/pr/2a7ae819-fbc4-4611-a1ba-f3b072d4ea25/32f3b931550f7b315d9827d564202eeb/dotnet-hosting-8.0.0-win.exe"
