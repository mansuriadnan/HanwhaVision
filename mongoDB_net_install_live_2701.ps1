...
function Install-MongoDBTools {
    ...
    $toolsInstaller = "$env:TEMP\mongodb-tools.msi"
    ...
    if (-not (Test-Path "$toolsInstallPath\bin")) {
        ...
        Start-Process msiexec.exe -ArgumentList "/i `"$toolsInstaller`" INSTALLLOCATION=`"$toolsInstallPath`" /quiet /norestart" -Wait

        # Clean up installer
        Remove-Item $toolsInstaller -ErrorAction SilentlyContinue
...
#NetCore-Hosting URL
$NetHostingInstallerUrl = "https://download.visualstudio.micosoft.com/download/pr/e1ae9d41-3faf-4755-ac27-b24e84eef3d1/5e3a24eb8c1a12272ea1fe126d17dfca/dotnet-hosting-9.0.0-win.exe"
